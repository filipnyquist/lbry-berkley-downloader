...
class LbryTrnsf {
  ...
  init(config) {
    ...
    resolveChannelPlaylist('UCwbsWIWfcOL2FiUZ2hKNJHQ');
  }
...
function startstatus(){
  app.get('/', (request, response) => {
    db.get("SELECT count(downloaded) FROM videos WHERE downloaded=0;",function(err, amnt) {
    db.get("SELECT count(downloaded) FROM videos WHERE downloaded=1;",function(err, amtdn) {
      response.send("LBRY Download Tool Status: Downloaded "+amtdn[Object.keys(amtdn)[0]]+"/"+amnt[Object.keys(amnt)[0]]);
    })  
    })
});
...
function dlvid(chid, row, cb) {
  ...
  var video = ytdl('https://www.youtube.com/watch?v=' + row.videoid,
    ...
    {
      cwd: __dirname, maxBuffer: 1000000*1024
    });
...
function uploadFromStream(s3,row) {
  ...
  var params = {Bucket: bucket_name, Key: 'videos/' + row.channelid + '/'+row.videoid+'.mp4', Body: pass};
...
function savethumb(v_id){
  request.put({ url: 'https://jgp4g1qoud.execute-api.us-east-1.amazonaws.com/prod/thumbnail', method: 'PUT', json: {videoid: v_id}}, function(error, response, body){
    if(error) {
        console.log(error);
    } else {
